...
class Validator {
     ...
     * @param array $checks
     * @param array $data
     * @param int $when MODEL_INSERT|MODEL_UPDATE
     * @return array
     */
    public static function check($checks, $data, $when) {
        ...
        foreach ($checks as $key => $check) {
            $which = VALIDATE_EXISTS;
            //没有设置 when,则每次都验证;设置了指定的验证时间,则只在满足指定验证时间时验证
            if (!empty($check['when'])) {
                if (isset($check['when'][$when])) {
                    $which = $check['when'][$when];
                } elseif (isset($check['when'][MODEL_BOTH])) {
                    $which = $check['when'][MODEL_BOTH];
                } else {
                    continue;
                }
            }
            //存在字段则验证
            if ($which == VALIDATE_EXISTS && !isset($data[$key])) {
                continue;
            }
            //值不为空则验证
            if ($which == VALIDATE_VALUE && empty($data[$key])) {
                continue;
            }
            //如果需要去除标签
