...
var runImg = {
    ...
    autoPlay: function(){
        ...
        that.setting.move = function(){
            ...
            $(that.setting.imgBox).find('li').eq(that.setting.index).fadeOut(300);
            $(that.setting.imgBox).find('li').eq(that.setting.next).fadeIn(300);
    ...
    slide: function(btn,imgBox,active){
        var that = this;
        that.mouseoverout(btn);
        $(btn).find('li').on('click',function(){
            var index = $(this).index();
            if(index == that.setting.index){
                return;
            }
            $(imgBox).find('li').eq(that.setting.index).fadeOut(300);
            $(imgBox).find('li').eq(index).fadeIn(300);
