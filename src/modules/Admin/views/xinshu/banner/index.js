;(function(window){
    window.banner = {
        ...
        getModuleInfo: function(){
            ...
            $.ajax({
                ...
                success:function(data){
                    if(data.status==1){
                        var value=data.data.showType == 0 ? '图文轮播' : '纯图轮播';
        ...
        chooseColor: function(){
            var that=this;
            that.element.a.keyup(function(){
                that.element.c.css({background:that.element.a.val()});
                that.element.b.val(that.element.a.val());
            });
            that.element.b.on('change',function(){
                that.element.c.css({background:that.element.b.val()});
                that.element.a.val(that.element.b.val());
            })
        },
        choose: function(val){
        ...
        save: function(){
            ...
            var value = this.opt.chooseVal == 0 ? '图文轮播' : '纯图轮播';
        ...
        getDataAction: function(page,row,order,status,regularlyStart,regularlyEnd){
            var that=this;
            $('#tt').datagrid('loading');
            $.ajax({
                type: "post",
        ...
        datagrid: function(){
            ...
            $('#dom_var_pagination').appendTo($('.datagrid-wrap'));
        },
        newUser: function(){
            var that=this;
            $('#dlg').window('open');
        ...
        doSearch: function(value){
            banner.opt.params.keyWords.description = value;
        ...
        setData:function(data){
            ...
            description=data.description?data.description:'点击查看详情';
            ...
            $('.dlg-color-box input').val(bgcolor);
            $('.dlg-color-box input').val(bgcolor);
            $('.dlg-color-bg').css({background:bgcolor});
            $('#linkurl').val(linkUrl);
