    ...
    window.app = {
            ...
            var that = this;
             that.element.table.datagrid({
                ...
                columns:[[
                    ...
                    {field: 'linkTitle', title: '文本', width: 150, align: 'center',formatter:function(value,row,index){
        ...
        validate: function(){
            ...
            that.element.addForm.validate({
                rules: {linkTitle: "required",linkUrl: {required:true,url:true}},
                messages: {linkTitle: "请输入超链接文本",linkUrl: {required:"请输入超链接",url:"请输入正确的超链接"}},
        ...
        addItemSubmit: function(startTime,endTime){
            ...
            var result = {
                isDefault : 0,
                linkTitle : that.element.linkTitle.val(),
                linkUrl : that.element.linkUrl.val(),
                startTime : startTime,
                endTime : endTime,
        ...
        initFormData: function(data){
            ...
            if(data){
                ...
                that.element.linkTitle.val(data.linkTitle);
                that.element.linkUrl.val(data.linkUrl);
                that.element.testLinkUrl.attr('href',data.linkUrl).text(data.linkUrl);
                ...
                that.element.auctionInfo.datagrid('loadData',auctionInfo);
                that.element.linkTitle.val('');
                that.element.linkUrl.val('');
                that.element.testLinkUrl.attr('href','').text('');
        ...
        search: function(value){
            app.opt.params.keyWords.linkTitle = value;
            app.getData();
        },
        bindChange: function(){
            var that = this;
            that.element.linkUrl.change(function(){
                var url = $(this).val();
                that.element.testLinkUrl.attr('href',url).text(url);
            });
        },
        run: function(){
            var that = this;
