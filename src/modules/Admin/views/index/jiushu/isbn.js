;(function(window){
    window.app = {
        ...
        getModuleInfo: function(){
            ...
            $.ajax({
                type: "post",
                url: "/admin/module/getModuleSyncInfo",
                dataType: "json",
                data:{
                ...
                success: function(res){
                    if(res.status == 1){
                        var type = '否';
                        if(res.data.params && res.data.params.sync.switch == '1'){
                            type = '是';
                        }
                        that.element.dataModuleName.text( res.data.syncModule.syncModuleArea + '-' + res.data.syncModule.title );
                        that.element.dataTypeText.text(type);
                        setTimeout(function(){
                            that.element.dataType.combobox('setValue',res.data.params.sync.switch);
                        });
                    }
                }
            })
        ...
        combobox: function(){
            ...
            that.element.dataType.combobox({
                valueField: 'label',
                textField: 'value',
                data:[
                    {label: '1',value: '同步'},
        ...
        addItemSubmit: function(startTime,endTime){
            ...
            if(that.opt.addItem.isbn === ''){
                that.alertInformation({type:false,msg:'请先点击查询按钮并检测ISBN商品信息'});
                return;
            }
            var result = {
                isDefault : 0,
        ...
        editDataType: function(){
            var that = this;
            that.element.dataWin.window('open');
        },
        // 确定修改数据源
        dataTypeSet: function(){
            var that = this,
                type = that.element.dataType.combobox('getValue');
            $.ajax({
                type: "post",
                url: "/admin/module/setSyncModule",
                dataType: "json",
                data:{
                    moduleId: that.opt.moduleId,
                    switch: type
                ...
                success:function(res){
                    if(res.status == 1){
                        that.element.dataWin.window('close');
                        that.alertInformation({type:true,msg:'操作成功'});
                        that.element.dataTypeText.text(type == '1'?'是':'否');
                        if(type == '1'){
                            that.element.table.datagrid('hideColumn','confId');
                        }else{
                            that.element.table.datagrid('showColumn','confId');
                        }
                        that.getData();
                    }else{
                        that.alertInformation({type:false,msg:res.message});
