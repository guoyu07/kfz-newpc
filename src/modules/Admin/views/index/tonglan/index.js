;(function(window){
    window.ad = {
        ...
        getTonglan: function(){
            ...
            $.ajax({
                ...
                success:function(data){
                    if(data.status==1){
                        $.each(data.data.params.tonglanConfig,function(index,ele){
                            var value=ele == 0 ? '显示' : '隐藏';
                            $('#tonglanShow').datagrid("getRows")[0][index]=value;
                            $('#tonglanShow').datagrid('refreshRow', 0);
                        })
                    }else {
        ...
        tlShow:function(index){
            $('#w').window('open');
            this.opt.tlShowIndex=index;
        },
        choose: function(val){
        ...
        save: function(){
            ...
            var val=that.opt.chooseVal == 0 ? '显示' : '隐藏';
            var values=$('#tonglanShow').datagrid("getRows")[0][that.opt.tlShowIndex];
            if(val == values){
                that.alertInformation({type:false,msg:'你已经选择了'+val})
                return;
            }
            $.ajax({
                type: "post",
                url: "/admin/module/editTonglan",
                dataType: "json",
                data:{
                    moduleId:that.opt.moduleId,
                    id:that.opt.tlShowIndex,
                    isHide:that.opt.chooseVal
                },
                success:function(data){
                    if(data.status==1){
                        $('#tonglanShow').datagrid("getRows")[0][that.opt.tlShowIndex]=val;
                        $('#tonglanShow').datagrid('refreshRow', 0);
                        $('#w').window('close');
