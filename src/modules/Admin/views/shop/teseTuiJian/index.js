;(function(window){
    window.app = {
            ...
            var that = this;
             that.element.table.datagrid({
                ...
                columns:[[
                    ...
                    {field: 'itemId', title: '商品ID', width: 150, align: 'center',formatter:function(value,row,index){
                        return '<a href="' + site.shop + value + '" target="_blank">' + value + '</a>';
                    }},
                    {field: 'itemName', title: '商品名称', width: 150, align: 'center',formatter:function(value,row,index){
        ...
        getShopInfo: function(){
            var that = this,
                shopId = that.element.shopId.val(),
                itemId = that.element.itemId.val();
            if(shopId == ''){
                that.alertInformation({type:false,msg:'请输入要查询的书店ID'});
                return;
            }
            if(itemId == ''){
                that.alertInformation({type:false,msg:'请输入要查询的书店ID'});
            ...
            $.ajax({
                type: "post",
                url: "/admin/shop/getItemInfoById",
                dataType: "json",
                data: {
                    shopId: shopId,
                    itemId: itemId
                ...
                success: function(res){
                    if(res.status == 1){
                        var data = [
                            {key : '商品主图',value : res.data.imgUrl},
                            {key : '商品名称',value : res.data.itemName,itemId : res.data.itemId},
                            {key : '商品ID',value : res.data.itemId},
                            {key : '商品分类',value : res.data.catName},
                            {key : '库存',value : res.data.number},
                            {key : '价格',value : res.data.price},
                            {key : '书店名称',value : res.data.shopName,shopId : res.data.shopId},
                            {key : '卖家信用',value : res.data.credit},
                            {key : '卖家好评率',value : res.data.goodRate}
                        ];
        ...
        addItemSubmit: function(startTime,endTime){
            var that = this;
            if( that.opt.addItem.shopName === '' || that.opt.addItem.itemName === ''){
                that.alertInformation({type:false,msg:'请先点击查询按钮并检测商品信息'});
                return;
            }
            var result = {
                isDefault : 0,
                ...
                dataId : that.element.itemId.val(),
        ...
        initFormData: function(data){
            ...
            if(data){
                var shopInfo = [
                    {key : '商品主图',value : data.imgUrl},
                    {key : '商品ID',value : data.itemId},
                    {key : '商品名称',value : data.itemName,itemId : data.itemId},
                    {key : '商品分类',value : data.catName},
                    {key : '库存',value : data.number},
                    {key : '价格',value : data.price},
                    {key : '书店名称',value : data.shopName,shopId : data.shopId},
                    {key : '卖家信用',value : data.rate},
                ...
                that.element.shopId.val(data.shopId).attr("disabled","disabled");
                that.element.itemId.val(data.itemId).attr("disabled","disabled");
                ...
                that.element.shopInfo.datagrid('loadData',shopInfo);
                that.element.shopId.val('').removeAttr("disabled");
                that.element.itemId.val('').removeAttr("disabled");
                that.element.tabType.combobox("setValue",that.opt.tabList[0].label);
        ...
        search: function(value){
            ...
            app.opt.params.keyWords.itemId = value;
            app.getData();
        },
        run: function(){
            var that = this;
