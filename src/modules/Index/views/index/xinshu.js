(function (root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        // AMD. 注册匿名模块
        define(['widgets/banner/full/banner'],['widgets/nav/xinshuleftbar/nav'], factory);
    } else {
        factory(root.widgets['banner/full/banner'],root.widgets['nav/xinshuleftbar/nav']);
    }
    ...
    var index_left={
        init:function(){
           $(window).scroll( function() {
                 var div_height = $('.xinshuLeftbar').height();
                 ...
                 if(change_fixed_height > wind_height){
                        ...
                        if(scrollTop_height > normol_top && change_height +205 <= wind_height){
                             ...
                             $('.xinshuLeftbar').css({position:"fixed",top:"auto",left:left_width,bottom:"205px",height:div_height});    
                        }
                       else{
                            $('.xinshuLeftbar').css({position:"relative",top:"0px",left:"0px",height:div_height});
                       }                    
                 }
                 else{
