...
start() {
    #webdav目录
    mkdir -p /data/webdav
    chown web:web /data/webdav
    cat /etc/hosts $project_dir/hosts > /etc/hosts.tmp
...
widget() {
    ...
    if [ $# -lt 1 ];
    then
         echo "请输入模块名称, 格式为'nav/top', 其中'nav'表示模块类型, 'top'表示模块名"
         read name
    else
        name=$1
    fi
    ...
    if [ -z $widgetName ]; then
        echo "模块名称格式错误, 正确的格式为'nav/top', 其中'nav'表示模块类型, 'top'表示模块名"
        exit 1
    fi
